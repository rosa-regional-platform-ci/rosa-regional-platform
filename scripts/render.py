...
def render_region_deployment_terraform(rd: Dict[str, Any], deploy_dir: Path) -> None:
    ...
    # Generate regional.json with nested terraform_vars
    regional_file = terraform_dir / "regional.json"
    tf_vars = rd.get("terraform_vars", {}).copy()

    # Add deterministic regional_id for resource naming
    tf_vars["regional_id"] = rd["regional_id"]

    # Add sector for tagging
    tf_vars["sector"] = rd.get("sector", environment)

    # Add environment_domain from environment_config when set
    env_domain = rd.get("environment_config", {}).get("domain")
    if env_domain:
        tf_vars["environment_domain"] = env_domain
    ...
    # Lifecycle flags at top level (outside terraform_vars)
    ...
    if management_clusters:
        ...
        for mc in management_clusters:
            ...
            mc_tf_vars = rd.get("terraform_vars", {}).copy()
            mc_tf_vars["sector"] = rd.get("sector", environment)

            # MC-specific TF var overrides (only real Terraform variables)
            mc_tf_overrides = {
                "management_id": management_id,
                "regional_id": rd["regional_id"],
                "regional_aws_account_id": rd.get("account_id"),
            }
            mc_tf_vars = deep_merge(mc_tf_vars, mc_tf_overrides)

            mc_data: Dict[str, Any] = {
                "_generated": "DO NOT EDIT - Generated by scripts/render.py from config.yaml",
                "terraform_vars": mc_tf_vars,
            }

            # account_id at top level — used by shell scripts for AWS credential
            # setup, not a Terraform variable
            if mc.get("account_id"):
                mc_data["account_id"] = mc["account_id"]

            # Lifecycle flags at top level (outside terraform_vars)
            if mc.get("delete") is True:
                mc_data["delete"] = True
            if mc.get("delete_pipeline") is True:
                mc_data["delete_pipeline"] = True
...
def render_environment_config(
    ...
    for rd in region_deployments:
        ...
        # Build management_clusters as dict with account_id per MC
        mc_dict: Dict[str, Any] = {}
        for mc in rd.get("management_clusters", []):
            mc_id = mc.get("management_id", "")
            mc_entry: Dict[str, Any] = {}
            if mc.get("account_id"):
                mc_entry["account_id"] = mc["account_id"]
            if mc.get("delete_pipeline") is True:
                mc_entry["delete_pipeline"] = True
            mc_dict[mc_id] = mc_entry

        region_entry: Dict[str, Any] = {
            "account_id": rd.get("account_id", ""),
            "regional_id": rd["regional_id"],
            "management_clusters": mc_dict,
        }

        # Add optional per-region fields
        tf_state_region = rd.get("terraform_vars", {}).get("tf_state_region")
        if tf_state_region:
            region_entry["tf_state_region"] = tf_state_region
        if rd.get("delete_pipeline") is True:
