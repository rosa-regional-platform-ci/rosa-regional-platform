...
CI jobs still use Vault-mounted credentials. These are managed in [Vault](https://vault.ci.openshift.org/ui/vault/secrets/kv/kv/list/selfservice/cluster-secrets-rosa-hyperfleet-int/):

| Secret                              | Used by                              | Key fields   |
| ----------------------------------- | ------------------------------------ | ------------ |
| `rosa-hyperfleet-ephemeral-creds`   | `nightly-ephemeral`, `on-demand-e2e` | `aws_config` |
| `rosa-hyperfleet-integration-creds` | `nightly-integration`                | `aws_config` |
