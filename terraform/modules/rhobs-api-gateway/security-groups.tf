...
resource "aws_vpc_security_group_egress_rule" "alb_to_loki_write" {
  security_group_id            = aws_security_group.alb.id
  description                  = "Allow traffic to Loki Write pods"
  ip_protocol                  = "tcp"
  from_port                    = var.loki_write_port
  to_port                      = var.loki_write_port
  referenced_security_group_id = var.node_security_group_id
}

resource "aws_vpc_security_group_egress_rule" "alb_to_loki_read" {
  count = var.loki_write_port != var.loki_read_port ? 1 : 0

  security_group_id            = aws_security_group.alb.id
  description                  = "Allow traffic to Loki Read pods"
  ip_protocol                  = "tcp"
  from_port                    = var.loki_read_port
  to_port                      = var.loki_read_port
  referenced_security_group_id = var.node_security_group_id
}

resource "aws_vpc_security_group_ingress_rule" "nodes_from_alb_loki_write" {
  security_group_id            = var.node_security_group_id
  description                  = "Allow RHOBS ALB traffic to Loki Write pods"
  ip_protocol                  = "tcp"
  from_port                    = var.loki_write_port
  to_port                      = var.loki_write_port
  referenced_security_group_id = aws_security_group.alb.id
...
resource "aws_vpc_security_group_ingress_rule" "nodes_from_alb_loki_read" {
  count = var.loki_write_port != var.loki_read_port ? 1 : 0

  security_group_id            = var.node_security_group_id
  description                  = "Allow RHOBS ALB traffic to Loki Read pods"
  ip_protocol                  = "tcp"
  from_port                    = var.loki_read_port
  to_port                      = var.loki_read_port
  referenced_security_group_id = aws_security_group.alb.id
