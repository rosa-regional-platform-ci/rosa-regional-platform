...
resource "aws_wafv2_web_acl" "api_gateway" {
  ...
  # Allow internal MC→RC API paths to bypass WAF managed rules.
  # Management cluster registration and internal API calls use IAM SigV4
  # auth and must not be blocked by content-inspection rules.
  rule {
    ...
    visibility_config {
      ...
      metric_name                = "${var.cluster_id}-allow-internal-api"
