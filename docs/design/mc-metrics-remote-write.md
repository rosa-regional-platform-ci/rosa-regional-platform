...
The API Gateway integration request template sets the `THANOS-TENANT` header server-side:

```
#set($context.requestOverride.header.THANOS-TENANT = $context.identity.accountId)
```

This ensures the tenant identity comes from the verified SigV4 caller identity, not from client-supplied headers. A compromised MC cannot write metrics under another MC's tenant ID.
...
- **Binary media types**: `application/x-protobuf` configured to handle binary payloads
- **Content handling**: `CONVERT_TO_BINARY` on the integration request and response
...
- sigv4-proxy does not log request-level activity at any log level — debugging requires API Gateway execution logs (must be enabled separately) or Prometheus remote_storage metrics
