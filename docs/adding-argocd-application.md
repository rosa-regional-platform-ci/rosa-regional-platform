...
```yaml
applications:
  # Wave 0:  Bootstrap — ArgoCD self-management and CRD operators
...
| `wave`         | Sync-wave (0, 10, 20). Controls deployment order — lower waves deploy first and must be healthy.      |
...
- **0** — ArgoCD self-management and CRD operators with zero dependencies (e.g., `external-secrets`, `prometheus-crds`)
- **10** — Configures wave-0 operators or installs secondary operators (e.g., `monitoring` with `crds.enabled: false`)
- **20** — All platform workloads and observability services (e.g., `platform-api`, `maestro-server`, `alerting-rules`)
