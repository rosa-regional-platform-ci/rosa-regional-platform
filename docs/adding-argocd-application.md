...
2. [Plumbing a Terraform value](#plumbing-a-terraform-value-to-argocd) through to an ArgoCD application
...
| `wave`         | Sync-wave (0, 10, 20, 30). Controls deployment order — lower waves deploy first and must be healthy.  |
...
- **0** — Installs CRDs or operators with zero dependencies (e.g., `external-secrets`, `prometheus-crds`)
- **10** — Configures wave-0 operators or installs secondary operators (e.g., `monitoring` with `crds.enabled: false`)
- **20** — Consumes CRDs from wave 0+10 (e.g., `alerting-rules`, `argocd` self-management)
- **30** — Platform workloads that depend on the infrastructure above (e.g., `platform-api`, `maestro-server`)
...
```text
Terraform output
  → bootstrap-argocd.sh (extracts output, passes as ECS env var)
    → ECS bootstrap task (writes to cluster Secret annotation)
      → ApplicationSet template (reads annotation into Helm values)
        → App-of-apps template (injects into child Application valuesObject)
          → Your child chart (.Values)
```
...
The `ClusterSecretStore` named `aws-secrets-manager` is deployed at wave 10 by `external-secrets-config`. Your app must be in wave 20 or later.
